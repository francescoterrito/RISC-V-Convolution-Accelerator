...
proc sram_power { name macro } {
    ...
    # Only add stripes if pitch is >= 20 (minimum required)
    # Small macros like 64x64 rely on power ring only
    if {$stripe_dist >= 20} {
        ...
        utl::report "Skipping stripes for $macro (pitch $stripe_dist < 20), using power ring only"
    }
...
# NOTE: Deep TopMetal2 to lower metal connections (Metal1, Metal2, Metal4) cause
# OpenROAD pdngen to segfault or hang. Commenting out these problematic connections.
# The power ring already connects TopMetal1/TopMetal2 via -add_connect flag.
# add_pdn_connect -grid {core_grid} -layers {TopMetal2 Metal1}  # CAUSES SEGFAULT
# add_pdn_connect -grid {core_grid} -layers {TopMetal2 Metal2}  # CAUSES SEGFAULT
# add_pdn_connect -grid {core_grid} -layers {TopMetal2 Metal4}  # CAUSES SEGFAULT
# add_pdn_connect -grid {core_grid} -layers {TopMetal2 TopMetal1}

# Power ring to standard cell rails (use shallower connections that work)
